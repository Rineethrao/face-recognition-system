...
class Settings:
    def __init__(self, json_path: Path = CONFIG_JSON_PATH):
        ...
        self.VISITOR_QUALITY_ENROLLMENT_THRESH: float = float(_vis.get("quality_enrollment_threshold", 0.62))
        self.VISITOR_QUALITY_AVATAR_THRESH: float = float(_vis.get("quality_avatar_threshold", 0.70))

        self.VISITOR_MIN_FACE_WIDTH: int = int(_vis.get("min_face_width", 28))
        self.VISITOR_MIN_FACE_HEIGHT: int = int(_vis.get("min_face_height", 28))
        self.VISITOR_ENROLLMENT_MIN_FACE_WIDTH: int = int(_vis.get("enrollment_min_face_width", 50))
        self.VISITOR_ENROLLMENT_MIN_FACE_HEIGHT: int = int(_vis.get("enrollment_min_face_height", 50))
        self.VISITOR_MAX_ABS_YAW: float = float(_vis.get("max_abs_yaw", 0.18))
        self.VISITOR_MAX_ABS_PITCH: float = float(_vis.get("max_abs_pitch", 0.15))
        self.VISITOR_MIN_BLUR_SCORE: float = float(_vis.get("min_blur_score", 40.0))
        self.VISITOR_ENROLLMENT_MIN_BLUR_SCORE: float = float(_vis.get("enrollment_min_blur_score", 55.0))
        self.VISITOR_MAX_BLUR_SCORE: float = float(_vis.get("max_blur_score", 800.0))
        self.VISITOR_MIN_DET_SCORE: float = float(_vis.get("min_det_score", 0.50))
        self.VISITOR_ENROLLMENT_MIN_DET_SCORE: float = float(_vis.get("enrollment_min_det_score", 0.60))
        self.VISITOR_MIN_STRUCTURE_SCORE: float = float(_vis.get("min_structure_score", 0.45))
        self.VISITOR_ENROLLMENT_MIN_STRUCTURE_SCORE: float = float(_vis.get("enrollment_min_structure_score", 0.55))
        self.VISITOR_ENROLLMENT_MIN_EYE_DISTANCE: float = float(_vis.get("enrollment_min_eye_distance", 18.0))

        self.VISITOR_MIN_OBSERVATIONS_FOR_NEW: int = int(_vis.get("min_observations_for_new_visitor", 6))
        self.VISITOR_MIN_AVATAR_FRAMES_FOR_NEW: int = int(_vis.get("min_avatar_frames_for_new", 2))
        self.VISITOR_MIN_MEAN_ENROLLMENT_QUALITY: float = float(_vis.get("min_mean_enrollment_quality", 0.62))
        self.VISITOR_MIN_BEST_FRAME_QUALITY: float = float(_vis.get("min_best_frame_quality", 0.68))
        self.VISITOR_MIN_TRACK_SPAN_SECONDS: float = float(_vis.get("min_track_span_seconds", 1.20))
        self.VISITOR_MIN_SAMPLE_GAP_SECONDS: float = float(_vis.get("min_sample_gap_seconds", 0.20))
        self.VISITOR_WITHIN_TRACK_CONSISTENCY_THRESH: float = float(_vis.get("within_track_consistency_thresh", 0.58))
        self.VISITOR_PENDING_CONFIRM_VOTES: int = int(_vis.get("pending_confirm_votes", 3))
        self.VISITOR_MAX_SAMPLES: int = int(_vis.get("max_samples_per_visitor", 6))
